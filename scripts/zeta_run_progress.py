...
def draw(stdscr):
    ...
    show_detail = False
    while True:
        master = parse_master_log(latest_file("distribute_T*_*.log"))
        ...
        stdscr.erase()
        h, w = stdscr.getmaxyx()
        title = "  ζ  ZETA RUN PROGRESS — PC1/PC2  ζ  "
        ...
        if master:
            ...
            stdscr.addstr(row, 2, "Aucun run distribué détecté (logs/distribute_T*.log absent).", curses.color_pair(PAIR["warn"]))
            row += 2
            n1 = n2 = None
        ...
        for label, name, done, n_assigned, elapsed, host in (
            ...
            if n_assigned:
                ...
                eta = (elapsed / done * (n_assigned - done)) if done > 0 else 0
                bar(stdscr, row, 4, pct, 30, PAIR[name], f"zéros {done}/{n_assigned} · ETA {eta/60:.1f} min")
            else:
