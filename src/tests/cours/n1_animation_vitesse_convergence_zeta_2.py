"""
Module niveau-1 : visualisation   vitesse de convergence de ζ ( s ) 
Connexion avec  ζ ( s )  
...
mp.dps = 25  # plus de précision pour que ζ(s) exact soit fiable
...
# ── MODIFICATION : erreur relative en échelle logarithmique ──────────────────
ax2 = axes[1]
N_MAX = 500
for s, color, ls in [(1.1, 'blue', '-'), (1.5, 'orange', '--'), (2, 'green', ':')]:
    exact = float(zeta(mpf(s)))
    errors = []
    S = mpf(0)
    for n in range(1, N_MAX + 1):
        S += mpf(1) / mpf(n)**s
        err = abs(float(S) - exact) / exact
        errors.append(err if err > 0 else 1e-30)
    ax2.semilogy(range(1, N_MAX + 1), errors, color=color, linestyle=ls,
                 linewidth=1.8, label=f's={s}  →  ζ(s)≈{exact:.4f}')
