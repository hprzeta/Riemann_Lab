...
# -*- coding: utf-8 -*-
"""
compute_zeros_v14.py — Phase C v14 : Illinois 1 Newton Z_arb + cache RS
═════════════════════════════════════════════════════════════════════════
v14 (2026-07-04) : 2 optimisations dans illinois_arb.c et scan_arb.c.

Diagnostic v13 T=100k (8.50 min, 271 z/s) :
  Illinois = 99.6% du temps (Phase 2 : 2 Newton Z_arb ≈ 3.64 ms/zéro)
  Scan     = 0.4% du temps (Z_double en C, quasi-gratuit)
  Z_arb    ≈ 1.8 ms/appel → 2 appels Phase 2 = 3.6 ms/zéro

Optimisations v14 :
  1. Phase 2 : 1 seul Newton Z_arb (au lieu de 2)
     Depuis |b-a|=1e-6 et convergence quadratique : 1 step suffit (erreur < 1e-12).
     Gain attendu : ×1.9 sur Illinois → T=100k : 8.5 → ~4.5 min.
  2. Cache log_n + isqrt_n dans scan_arb.c et illinois_arb.c :
     Évite log(n) et 1/sqrt(n) répétés à chaque terme RS.
     Gain sur Phase 1 et scan : ×1.3-2.

Invariants conservés depuis v13 :
  Phase 1 : Illinois Z_rs C (cache) → |b-a| < 1e-6 (inchangé)
  T_SEUIL_PETIT_T : 65.0 (inchangé)
  TOL_ARB : 1e-12 (inchangé — 1e-9 causait 16/20 LMFDB)
...
def worker_v14(args: tuple) -> Tuple[list, dict, dict]:
    """Worker v14 — détection scan_arb.c + affinage illinois_refine_arb.
...
def calculer_zeros_v14(
    ...
    with multiprocessing.Pool(processes=N_WORKERS) as pool:
        resultats = pool.map(worker_v14, args_list)
...
def rescan_segments_deficit(
    ...
    with multiprocessing.Pool(processes=len(args_rescan)) as pool:
        resultats_rescan = pool.map(worker_v14, args_rescan)
...
def visualiser(zeros: List[float], T_MAX: float, horodatage: str, dossier: Path):
    ...
    plt.tight_layout()
    nom_png = f"zeros_v14_T{T_MAX:.0f}_{horodatage}.png"
...
def sauvegarder_csv(zeros, stats, T_MAX, STEP, N_WORKERS,
    ...
    nom        = f"zeros_v14_T{T_MAX:.0f}_{horodatage}.csv"
...
def ecrire_log(chemin_log, horodatage, T_MIN, T_MAX, STEP, N_WORKERS,
    ...
    L(sep)
    L("  JOURNAL D'EXÉCUTION — compute_zeros_v14.py  (Phase C)")
...
def saisir_parametres():
    ...
    dossier    = Path("calculs") / f"v14_T{T_MAX:.0f}_{horodatage}"
...
def main():
    ...
    if _cli.t_min is not None and _cli.t_max is not None:
        ...
        dossier    = Path("calculs") / f"v14_T{T_MIN:.0f}_{T_MAX:.0f}_{horodatage}"
        dossier.mkdir(parents=True, exist_ok=True)
    ...
    zeros, stats, profil_workers, segments = calculer_zeros_v14(
    ...
    print()
    print("=" * 65)
    print("  RÉSULTATS v14 — run principal")
    print("=" * 65)
    ...
    nom_log    = f"execution_v14_T{T_MAX:.0f}_{horodatage}.log"
    ...
    print()
    print("=" * 65)
    print(f"  v14 terminée — fichiers dans : {dossier}")
