...
# -*- coding: utf-8 -*-
"""
compute_zeros_v13.py — Phase C v13 : seuil petit-t abaissé à 20
═══════════════════════════════════════════════════════════════════
v13 (2026-06-17) : correction du bottleneck Worker 0 découvert lors du
  benchmark v12 PC1/PC2 — T_SEUIL_PETIT_T abaissé de 200 à 20.

Diagnostic v12 T=1000 :
  PC1 (python-flint présent)  : Worker 0 = 2.9 s    → total 3.3 s
  PC2 (python-flint absent)   : Worker 0 = 101.2 s  → total 101.3 s
  Cause : 18 600 appels mpmath.siegelz (fallback arb_hardy_z) dans [14, 200]

Correction v13 :
  T_SEUIL_PETIT_T = 20  (était 200 dans v12)
  θ(t) asymptotique valide dès t ≥ 20 (Abramowitz & Stegun 6.1.40)
  Pour t ∈ [14, 20] : ~600 évaluations mpmath → ~3 s même sans python-flint
  Pour t ∈ [20, 200] : scan_arb C (Z_double + C0+C1), détection ×35 plus rapide
  Spurieux éventuels filtrés par la borne [a, b] d'illinois_refine_arb (Arb).

Signature ctypes scan_zeros_arb — 8 arguments (cf. scan_arb.c) :
  ...
  Erreur connue : passer 2 POINTER au lieu de 4 → retourne 0 silencieusement
  (ctypes ne lève pas d'exception — comportement indéfini C).
  Wrapper corrigé : scan_arb_wrapper.py (chargé via `from scan_arb_wrapper import scan_arb`).

Invariants conservés depuis v12 :
...
def worker_v13(args: tuple) -> Tuple[list, dict, dict]:
    """Worker v13 — détection scan_arb.c + affinage illinois_refine_arb.
...
def calculer_zeros_v13(
    ...
    with multiprocessing.Pool(processes=N_WORKERS) as pool:
        resultats = pool.map(worker_v13, args_list)
...
def visualiser(zeros: List[float], T_MAX: float, horodatage: str, dossier: Path):
    ...
    fig.suptitle(
        f"Zéros de ζ(½+it) — v13 — {len(zeros)} zéros [T_MAX={T_MAX:.0f}]",
    ...
    plt.tight_layout()
    nom_png = f"zeros_v13_T{T_MAX:.0f}_{horodatage}.png"
...
def sauvegarder_csv(zeros, stats, T_MAX, STEP, N_WORKERS,
    ...
    nom        = f"zeros_v13_T{T_MAX:.0f}_{horodatage}.csv"
...
def ecrire_log(chemin_log, horodatage, T_MIN, T_MAX, STEP, N_WORKERS,
    ...
    L(sep)
    L("  JOURNAL D'EXÉCUTION — compute_zeros_v13.py  (Phase C)")
...
def saisir_parametres():
    print()
    print("=" * 65)
    print("   CALCUL DES ZÉROS NON TRIVIAUX — v13 (Phase C / Arb)")
    print("=" * 65)
    print()
    ...
    dossier    = Path("calculs") / f"v13_T{T_MAX:.0f}_{horodatage}"
...
def main():
    ...
    nom_log    = f"execution_v13_T{T_MAX:.0f}_{horodatage}.log"
