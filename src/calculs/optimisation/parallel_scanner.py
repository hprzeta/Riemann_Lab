...
# ─── Imports locaux ───────────────────────────────────────────────────────────
try:
    from riemann_siegel_batch import scanner_batch as scanner_segment
    from theta_rapide import Z_fast
    print("  ✅  Mode batch numpy activé (×7 à ×15 plus rapide)")
except ImportError:
    raise ImportError(
        "riemann_siegel_batch.py introuvable — "
        "le placer dans le même dossier que parallel_scanner.py"
    )
